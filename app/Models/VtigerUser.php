...
class VtigerUser extends Authenticatable
{
    ...
    {
        // Never let a cache-layer failure (e.g. unwritable storage/framework/cache) break login.
        try {
            return Cache::remember('agile_allowed_modules_v2_' . $this->id, 600, function () {
                return $this->fetchAllowedModules();
            });
        } catch (\Throwable $e) {
            \Illuminate\Support\Facades\Log::warning('getAllowedModules cache failed, falling back to live fetch', ['error' => $e->getMessage()]);
            return $this->fetchAllowedModules();
        }
    }
