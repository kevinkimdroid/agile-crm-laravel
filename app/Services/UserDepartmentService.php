...
class UserDepartmentService
{
    ...
    /** Per-request memo: avoids re-querying the (remote) DB for the same user on every menu/permission check. */
    protected static array $departmentMemo = [];
    ...
    public function getDepartment(?int $userId): ?string
    {
        ...
        if (array_key_exists($userId, self::$departmentMemo)) {
            return self::$departmentMemo[$userId];
        }
        ...
        if ($dept === null) {
            $dept = config('users.departments.' . $userId);
        }

        return self::$departmentMemo[$userId] = $dept;
    }
    ...
    public function setDepartment(int $userId, string $department): void
    {
        ...
        unset(self::$departmentMemo[$userId]);
    }
