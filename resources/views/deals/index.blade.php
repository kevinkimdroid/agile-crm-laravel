...
<div class="deals-page">
    <div class="deals-hero mb-4">
        <div class="d-flex flex-wrap align-items-start justify-content-between gap-3">
            ...
            <a href="{{ route('deals.create') }}" class="btn btn-light fw-semibold">
    ...
    <div class="row g-3 mb-4">
        <div class="col-sm-6 col-xl-3">
            <div class="deals-kpi deals-kpi-featured">
                ...
                <span class="deals-kpi-value">KES {{ number_format($pipelineValue ?? 0, 0) }}</span>
                <span class="deals-kpi-meta">{{ number_format($openCount ?? 0) }} open {{ ($openCount ?? 0) === 1 ? 'deal' : 'deals' }}</span>
            </div>
        </div>
        <div class="col-sm-6 col-xl-3">
            <div class="deals-kpi">
                <span class="deals-kpi-label">All deals</span>
                <span class="deals-kpi-value">{{ number_format($allCount) }}</span>
        ...
        <div class="col-sm-6 col-xl-3">
            <div class="deals-kpi">
                ...
                <span class="deals-kpi-value">{{ number_format($closingSoon->count()) }}</span>
        ...
        <div class="col-sm-6 col-xl-3">
            <div class="deals-kpi">
                ...
                <span class="deals-kpi-value">KES {{ number_format($wonValue ?? 0, 0) }}</span>
                <span class="deals-kpi-meta">{{ number_format($wonCount ?? 0) }} won</span>
            </div>
        </div>
    </div>

    <div class="deals-pills mb-4">
        <a href="{{ route('deals.index', array_filter(['q' => $search ?: null])) }}" class="deals-pill {{ $currentStage === '' ? 'active' : '' }}">
            All <span class="deals-pill-count">{{ number_format($allCount) }}</span>
        </a>
        @foreach ($stageOrder as $name)
            @php $row = $stageSummary[$name] ?? ['count' => 0, 'amount' => 0]; @endphp
            @if ($row['count'] > 0 || in_array($name, ['Prospecting', 'Qualification', 'Proposal', 'Negotiation', 'Closed Won', 'Closed Lost'], true))
            <a href="{{ route('deals.index', array_filter(['stage' => $name, 'q' => $search ?: null])) }}"
               class="deals-pill deals-pill-{{ Str::slug($name) }} {{ $currentStage === $name ? 'active' : '' }}">
                {{ $name }} <span class="deals-pill-count">{{ number_format($row['count']) }}</span>
            </a>
            @endif
        @endforeach
    </div>
    ...
    <div class="app-card deals-table-card overflow-hidden">
        <form method="GET" action="{{ route('deals.index') }}" class="deals-toolbar">
            @if ($currentStage !== '')
                <input type="hidden" name="stage" value="{{ $currentStage }}">
            @endif
            <div class="deals-search">
                <i class="bi bi-search"></i>
                <input type="text" name="q" class="form-control border-0 bg-transparent" placeholder="Search deal name, number, or stage…" value="{{ $search }}" autocomplete="off">
            </div>
            <button type="submit" class="btn btn-sm deals-search-btn">Search</button>
            @if ($search !== '' || $currentStage !== '')
                <a href="{{ route('deals.index') }}" class="btn btn-sm btn-outline-secondary">Clear</a>
            @endif
            <a href="{{ route('deals.create') }}" class="btn btn-sm app-topbar-add ms-auto">Add Deal</a>
        </form>
            ...
            <table class="table table-hover deals-table mb-0 align-middle">
                ...
                <tbody>
                    @forelse ($deals as $deal)
                        ...
                        <tr>
                            <td>
                                <a href="{{ route('deals.show', $deal->potentialid) }}" class="deals-name-link">
                                    <span class="deals-avatar">{{ $initials }}</span>
                                    <span>
                                        <span class="fw-semibold d-block">{{ $name }}</span>
                                        @if (!empty($deal->potential_no))
                                            <span class="deals-meta">{{ $deal->potential_no }}</span>
                                        @endif
                                    </span>
                                </a>
                            </td>
                            <td class="text-end deals-amount">KES {{ number_format($deal->amount ?? 0, 0) }}</td>
                            <td>
                                <span class="deals-stage deals-stage-{{ Str::slug($stageName) }}">{{ $stageName }}</span>
                            </td>
                            <td>
                                @if ($close)
                                    <span class="{{ $isOverdue ? 'deals-date-overdue' : ($isSoon ? 'deals-date-soon' : 'text-muted') }}">
                                        {{ $close->format('d M Y') }}
                                        @if ($isOverdue)
                            ...
                            <td class="text-end">
                                <div class="btn-group btn-group-sm">
                                    <a href="{{ route('deals.show', $deal->potentialid) }}" class="btn btn-outline-secondary" title="View"><i class="bi bi-eye"></i></a>
                                    <a href="{{ route('deals.edit', $deal->potentialid) }}" class="btn btn-outline-secondary" title="Edit"><i class="bi bi-pencil"></i></a>
                                </div>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="text-center py-5">
                                <div class="deals-empty">
                                    ...
                                    <h6 class="mt-3 mb-2">{{ $search !== '' || $currentStage !== '' ? 'No matching deals' : 'No deals yet' }}</h6>
                                    ...
                                    @if ($search !== '' || $currentStage !== '')
                                        <a href="{{ route('deals.index') }}" class="btn btn-outline-secondary">Clear filters</a>
                                    @else
                                        <a href="{{ route('deals.create') }}" class="btn btn-primary-custom"><i class="bi bi-plus-lg me-1"></i>New Deal</a>
                                    @endif
                                </div>
                            </td>
                        </tr>
        ...
        @if ($deals->hasPages())
            <div class="card-footer bg-transparent border-top py-3 d-flex flex-wrap justify-content-between align-items-center gap-2 px-4">
                <span class="text-muted small">Showing {{ $deals->firstItem() ?? 0 }}–{{ $deals->lastItem() ?? 0 }} of {{ number_format($deals->total()) }}</span>
