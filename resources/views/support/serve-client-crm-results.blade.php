...
@php
    $name = $item['name'] ?? 'Prospect #' . ($item['contactid'] ?? '');
    $meta = implode(' · ', array_filter([$item['phone'] ?? '', $item['email'] ?? '']));
    $openUrl = route('contacts.show', ['contact' => $item['contactid'], 'tab' => 'summary']);
@endphp
<div class="serve-client-item" data-open-url="{{ $openUrl }}">
...
<div class="serve-client-empty-detail py-4">
    <p class="mb-2 text-muted small"><i class="bi bi-person me-1"></i> No CRM prospects found{{ !empty($searchTerm ?? '') ? ' for "' . e($searchTerm ?? '') . '"' : '' }}.</p>
