# Agile Craft CRM – CentOS 7 Server Deployment
...
pip install -r /var/www/agile-crm-laravel/erp-clients-api/requirements.txt
...
```bash
cd /var/www
git clone https://your-repo/agile-crm-laravel.git
...
# scp -r /path/to/agile-crm-laravel user@server:/var/www/
# Then on server:
...
Create `/etc/nginx/conf.d/agile-crm.conf`:

```nginx
server {
    listen 80;
    server_name your-domain.com;   # or IP
    root /var/www/agile-crm-laravel/public;
...
Create `/etc/systemd/system/agile-crm-queue.service`:

```ini
[Unit]
Description=Agile Craft CRM Queue Worker
...
User=nginx
Group=nginx
WorkingDirectory=/var/www/agile-crm-laravel
...
systemctl enable agile-crm-queue
systemctl start agile-crm-queue
systemctl status agile-crm-queue
```
...
```ini
[Unit]
Description=Agile Craft ERP Clients API (Python)
...
User=nginx
Group=nginx
WorkingDirectory=/var/www/agile-crm-laravel/erp-clients-api
...
```cron
* * * * * cd /var/www/agile-crm-laravel && /opt/remi/php81/root/usr/bin/php artisan schedule:run >> /dev/null 2>&1
```
...
| Queue worker     | `systemctl start agile-crm-queue`                       |
...
- `deploy/systemd/agile-crm-queue.service` – queue worker
- `deploy/systemd/geminia-erp-api.service` – ERP API
- `deploy/nginx/agile-crm.conf` – Nginx config
