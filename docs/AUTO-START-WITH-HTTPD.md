...
2. Create Basic Task → Name: `Agile Craft CRM - Start Queue & API`
3. Trigger: **At startup** (or: **When the computer starts**)
4. Action: **Start a program**
5. Program: `C:\xampp\htdocs\sites\agile-crm-laravel\scripts\start-with-httpd.bat`
...
```batch
REM Queue worker
nssm install Agile CraftQueue "C:\xampp\php\php.exe" "artisan queue:work database --tries=3 --sleep=3"
nssm set Agile CraftQueue AppDirectory "C:\xampp\htdocs\sites\agile-crm-laravel"
nssm start Agile CraftQueue

REM ERP API
nssm install Agile CraftErpApi "C:\xampp\python\python.exe" "app.py"
nssm set Agile CraftErpApi AppDirectory "C:\xampp\htdocs\sites\agile-crm-laravel\erp-clients-api"
nssm start Agile CraftErpApi
```
...
- Start in: `C:\xampp\htdocs\sites\agile-crm-laravel`
...
```bash
cd /var/www/agile-crm-laravel

# Copy service files (configured for httpd)
sudo cp deploy/systemd/agile-crm-queue.service /etc/systemd/system/
sudo cp deploy/systemd/geminia-erp-api.service /etc/systemd/system/

# Edit WorkingDirectory if app is not at /var/www/agile-crm-laravel
# sudo nano /etc/systemd/system/agile-crm-queue.service

sudo systemctl daemon-reload
sudo systemctl enable agile-crm-queue geminia-erp-api
sudo systemctl start agile-crm-queue geminia-erp-api
```
...
```bash
* * * * * cd /var/www/agile-crm-laravel && php artisan schedule:run >> /dev/null 2>&1
```
